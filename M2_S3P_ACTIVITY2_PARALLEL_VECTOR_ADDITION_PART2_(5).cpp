...
int main() {
    ...
    // TRY DIFFERENT OPENMP SCHEDULING TECHNIQUES AND CHUNK SIZES
    #pragma omp parallel default(none) shared(v1, v2, v3, size) private(total)
    {
        int privateTotal = 0; // PRIVATE VARIABLE FOR EACH THREAD

        // SPECIFY CHUNK SIZE HERE
        int chunk_size = 1000; // ADJUST CHUNK SIZE HERE

        // EXPERIMENTING WITH DIFFERENT SCHEDULING TECHNIQUES AND CHUNK SIZES
